...
import lerDadosFuncionarios from './funcoes/funcionarios.js'
import cadastrarNovoFuncionario from './funcoes/cadastrarFuncionario.js';
import atualizarFuncionario from './funcoes/atualizarFuncinario.js';
...
const server = http.createServer((request, response) => {
    ...
    request.on('end', () => {
        ...
        atualizarFuncionario(id, novosDados, (err, funcionarioAtualizado) => {
            if (err) {
                response.writeHead(500, { 'Content-Type': 'application/json' });
                response.end(JSON.stringify({ message: 'Erro interno no servidor' }));
                return;
